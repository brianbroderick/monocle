defmodule Monocle.CLI do
  ...
  defp process(:version) do
    IO.puts( Monocle.version() )
  end

  defp process({io_device, options}) do
    options = struct(Monocle.Options, booleanify(options))
    content = IO.stream(io_device, :line) |> Enum.to_list
    Monocle.as_html!(content, options)
    |> IO.puts
  end
  ...
  defp booleanify_option({k, v}) do
    {k,
     case Map.get %Monocle.Options{}, k, :does_not_exist do
  ...
  defp specific_option_help( option ) do
    "      --#{option} defaults to #{inspect(Map.get(%Monocle.Options{}, option))}"
  end
