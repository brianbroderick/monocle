defmodule Monocle.Plugin do
  ...
  When invoking `Monocle.as_html(some_md, options)` we can register plugins inside the `plugins` map, where
  ...
  `%Monocle.Options{plugins: %{"" => CommentPlugin}}` would trigger the `CommentPlugin` for each block of
  lines prefixed by `$$`, while `%Monocle.Options{plugins: %{"cp" => CommentPlugin}}` would do the same for
  ...
  `Monocle.Options` structs for the usage of plugins.
  ...
  Monocle's render function will invoke the plugin's render function as explained above. It can then integrate the
  ...
  #### A complete example
        ...
        ...>   "$$ line two",
        ...> ]
        ...> Monocle.as_html(lines, Monocle.Plugin.define(MyPlug))
        {:error, "<h1>Plugin Ahead</h1>\\n<p>first line</p>\\n<hr/>", [{ :error, 4, "line two"}]}

  #### Plugins, reusing Monocle

  As long as you avoid endless recursion there is absolutely no problem to call `Monocle.as_html` in your plugin, consider the following
  example in which the plugin will parse markdown and render html verbatim (which is stupid, that is what Monocle already does for you,
  but just to demonstrate the possibilities):

        iex> defmodule Again do
        ...>   def as_html(lines) do
        ...>     text_lines = Enum.map(lines, fn {str, _} -> str end)
        ...>     {_, html, errors} = Monocle.as_html(text_lines)
        ...
        ...>    "$$a * two",
        ...>  ]
        ...>  Monocle.as_html(lines, Monocle.Plugin.define({Again, "a"}))
        {:ok, "<ul>\\n<li>one\\n</li>\\n<li>two\\n</li>\\n</ul>\\n* one* two", []}
  """

  @doc """
  adds the definition of one or more plugins to `Monocle.Options`.

  If the plugin is defined with the default prefix and no other options are needed
  one can use the one parameter form:

      iex> Monocle.Plugin.define(Monocle) # not a legal plugin of course
      %Monocle.Options{plugins: %{"" => Monocle}}

  More then one plugin can be defined, as long as all prefixes differ

      iex> defmodule P1, do: nil
      ...> defmodule P2, do: nil
      ...> Monocle.Plugin.define([ Monocle, {P1, "p1"}, {P2, "p2"} ])
      %Monocle.Options{plugins: %{"" => Monocle, "p1" => Unit.PluginTest.P1, "p2" => Unit.PluginTest.P2}}
