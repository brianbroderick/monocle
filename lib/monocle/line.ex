defmodule Monocle.Line do

  alias Monocle.Helpers
...
#'
  ...
  @spec scan_lines( list(String.t), %Monocle.Options{}, boolean ) :: ts
  def scan_lines lines, options \\ %Monocle.Options{}, recursive \\ false
  def scan_lines lines, options, recursive do
    lines_with_count( lines, options.line - 1)
    |> Monocle.pmap( fn (line) ->  type_of(line, options, recursive) end)
  end
  ...
  when is_boolean(recursive), do: type_of(line, %Monocle.Options{}, recursive)

  def type_of({line, lnb}, options = %Monocle.Options{}, recursive) do
  ...
  defp _type_of(line, options=%Monocle.Options{}, recursive) do
    cond do
      line =~ ~r/^\s*$/ ->
        %Blank{}
