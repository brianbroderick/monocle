defmodule Monocle do
  ...
  ### API
      ...
      * `Monocle.as_html`
        {:ok, html_doc, []}                = Monocle.as_html(markdown)
        {:error, html_doc, error_messages} = Monocle.as_html(markdown)

      * `Monocle.as_html!`
        html_doc = Monocle.as_html!(markdown, options)
  ...
  Options can be passed into `as_html` or `as_html!` according to the documentation.
        ...
        html_doc = Monocle.as_html!(markdown)
  ...
  will call
      ...
      Monocle.as_html!( ..., %Monocle.Options{smartypants: false, code_class_prefix: "a- b-"})
  ...
  * Rendering of block and inline elements.
  ...
  as a `code_class_prefix` to `Monocle.Options`.

  In the following example we want more than one additional class, so we add more prefixes.

        Monocle.as_html!(..., %Monocle.Options{code_class_prefix: "lang- language-"})
  ...
  ## Security
    ...
    filter the output of `Monocle.as_html` if you cannot trust the input
  ...
  import Monocle.Message, only: [emit_messages: 1, sort_messages: 1]
  ...
  The options are a `%Monocle.Options{}` structure:

  * `renderer`: ModuleName

    The module used to render the final document. Defaults to
    `Monocle.HtmlRenderer`
  ...
  * `smartypants`: boolean
  ...
  you'd call
      ...
      alias Monocle.Options
      Monocle.as_html(original, %Options{smartypants: false})
  ...
  The options are a `%Monocle.Options{}` structure. See `as_html!`
  for more details.
  """

  @spec parse(String.t | list(String.t), %Options{}) :: { Monocle.Block.ts, %Context{} }
  def parse(lines, options \\ %Monocle.Options{})
  def parse(lines, options = %Options{mapper: mapper}) when is_list(lines) do
    { blocks, links, options1 } = Monocle.Parser.parse(lines, options, false)

    context = %Monocle.Context{options: options1, links: links }
              |> Monocle.Context.update_context()

    if options.footnotes do
      { blocks, footnotes, options1 } = Monocle.Parser.handle_footnotes(blocks, context.options, mapper)
      context =
  ...
  @doc """
    Accesses current hex version of the `Monocle` application. Convenience for
    `iex` usage.
  """
  def version() do
  ...
  @doc false
  @spec pmap( list(A), (A -> Monocle.Line.t) ) :: Monocle.Line.ts
