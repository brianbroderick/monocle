defmodule FootnoteTest do
  ...
  def options do
    %Monocle.Options{footnotes: true, file: filename()}
  end

  def context do
    ctx = put_in(%Monocle.Context{}.options, options())
  ...
  test "handles multi-paragraph footnote bodies" do
    ...
    html = Monocle.as_html!(lines, put_in(%Monocle.Options{}.footnotes, true))
    expected_html = """
  ...
  test "parses footnote content" do
    ...
    fn_content = [%Monocle.Block.Para{lnb: 3, lines: ["line 1", "line 2", "line 3"]}]
    fn_def = %Monocle.Block.FnDef{lnb: 3, id: "ref-id", number: 1, blocks: fn_content }
    assert blocks == [%Monocle.Block.Para{lnb: 1, lines: ["para[^ref-id]"]},
                      %Monocle.Block.Para{lnb: 7, lines: ["para"]},
                      %Monocle.Block.FnList{lnb: 3, blocks: [fn_def]}
                     ]
  ...
  test "renders footnotes" do
    ...
    [^ref-a]: Ref A.
    """
    result = Monocle.as_html!(body, put_in(%Monocle.Options{}.footnotes, true))
    expected = """
